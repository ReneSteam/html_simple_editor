...
import 'package:html_simple_editor/src/widgets/webviewx/src/utils/constants.dart';
import 'package:html_simple_editor/src/widgets/webviewx/src/utils/embedded_js_content.dart';
...
class HtmlUtils {
  ...
  static String preprocessSource(
    ...
    if (!isFullHtmlPage(localSrc)) {
      localSrc = wrapHtml(localSrc, windowDisambiguator);
    }

    if (forWeb) {
      localSrc = embedWebIframeJsConnector(localSrc, windowDisambiguator!);
    }
    ...
    if (jsContent.isNotEmpty) {
      ...
      localSrc = embedJsInHtmlSource(localSrc, jsContentStrings);
    }

    if (encodeHtml) {
      localSrc = encodeHtmlToURI(localSrc);
    }
